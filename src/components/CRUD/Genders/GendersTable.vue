  ...
  <router-link :to="$route.fullPath + '/new'">
    ...
    >
      ...
      <template #actions="props">
        <div>
          <router-link :to="`/admin/genders/${props.row.id}/edit`">
...
export default {
  ...
  computed: {
    ...mapState({
      loading: state => state.genders.loading,
      tableData: state => state.genders.rows,
      modalOpen: state => state.genders.modalOpen,
      deleteId: state => state.genders.deleteId,
    }),
    dataFormatter() {
  ...
  methods: {
    ...mapActions({
      getData: 'genders/getData',
      deleteItem: 'genders/deleteItem',
    }),
    ...mapMutations({
      setDeleteId: 'genders/setDeleteId'
    }),
    del() {
