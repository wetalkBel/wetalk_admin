  ...
  <router-link :to="$route.fullPath + '/new'">
    ...
    >
      ...
      <template #actions="props">
        <div>
          <router-link :to="`/admin/tags/${props.row.id}/edit`">
...
export default {
  ...
  computed: {
    ...mapState({
      ...
      tableData: state => state.tags.rows,
      modalOpen: state => state.tags.modalOpen,
      deleteId: state => state.tags.deleteId,
    }),
    dataFormatter() {
  ...
  methods: {
    ...mapActions({
      getData: 'tags/getData',
      deleteItem: 'tags/deleteItem',
    }),
    ...mapMutations({
      setDeleteId: 'tags/setDeleteId'
    }),
    del() {
