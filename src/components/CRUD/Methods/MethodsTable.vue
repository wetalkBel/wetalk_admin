  ...
  <router-link :to="$route.fullPath + '/new'">
    ...
    >
      ...
      <template #actions="props">
        <div>
          <router-link :to="`/admin/methods/${props.row.id}/edit`">
...
export default {
  ...
  computed: {
    ...mapState({
      loading: state => state.methods.loading,
      tableData: state => state.methods.rows,
      modalOpen: state => state.methods.modalOpen,
      deleteId: state => state.methods.deleteId,
    }),
    dataFormatter() {
  ...
  methods: {
    ...
    ...mapMutations({
      setDeleteId: 'methods/setDeleteId'
    }),
    del() {
