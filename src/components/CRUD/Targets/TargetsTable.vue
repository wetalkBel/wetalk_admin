  ...
  <router-link :to="$route.fullPath + '/new'">
    ...
    >
      ...
      <template #actions="props">
        <div>
          <router-link :to="`/admin/targets/${props.row.id}/edit`">
...
export default {
  ...
  computed: {
    ...mapState({
      ...
      tableData: state => state.targets.rows,
      modalOpen: state => state.targets.modalOpen,
      deleteId: state => state.targets.deleteId,
    }),
    dataFormatter() {
  ...
  methods: {
    ...mapActions({
      getData: 'targets/getData',
      deleteItem: 'targets/deleteItem',
    }),
    ...mapMutations({
      setDeleteId: 'targets/setDeleteId'
    }),
    del() {
