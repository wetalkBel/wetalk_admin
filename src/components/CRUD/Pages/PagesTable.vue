  ...
  <router-link :to="$route.fullPath + '/new'">
    ...
    >
      ...
      <template #actions="props">
        <div>
          <router-link :to="`/admin/pages/${props.row.id}/edit`">
...
export default {
  ...
  computed: {
    ...mapState({
      ...
      tableData: state => state.pages.rows,
      modalOpen: state => state.pages.modalOpen,
      deleteId: state => state.pages.deleteId,
    }),
    dataFormatter() {
  ...
  methods: {
    ...mapActions({
      ...
      deleteItem: 'pages/deleteItem',
    }),
    ...mapMutations({
      setDeleteId: 'pages/setDeleteId'
    }),
    del() {
